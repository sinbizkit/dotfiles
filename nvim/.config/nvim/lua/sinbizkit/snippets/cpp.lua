...
{}
	...
	s("for", { t "for (size_t i = 0, size = ", i(1, "arr"), t({ ".size(); i < size; ++i) {", "\t" }),
		i(0),
		t({ "", "}" })
	}),

	s("foe", { t "for (const auto &val: ", i(1, "arr"), t({ ") {", "\t" }),
		i(0),
		t({ "", "}" })
	}),

	s("foi", { t "for (auto it = std::begin(", i(1, "arr"), t "), e = std::end(", rep(1), t({ "); it != e; ++it) {", "\t" }),
		i(0),
		t({ "", "}" })
	}),
	...
	s("fun", {
		i(3, "void"), t " ", i(1, "function"), t "(", i(2), t({ ") {", "\t" }),
		i(0), t({ "", "}" })
	}),
	...
	s("todo", fmt("//{}({}: {}) {}", {
		...
		f(function() return git_user_email() end),
		f(function() return os.date("%d/%m/%y") end),
		i(0, "description"),
	})),
